#ifndef _RPY_PLGN_HH_
#define _RPY_PLGN_HH_
...
namespace gazebo
{
  class RPYPlugin : public ModelPlugin
  {
    ...
    //XYZ varables
        ...
        ros::Publisher rawRPYPub, filtRPYPub;
        int frame, VBL, ABL;
    ...
    //____________________________________________________________ROS_node___________________________________________________________
        this->updateConnection = event::Events::ConnectWorldUpdateBegin(
            std::bind(&RPYPlugin::OnUpdate, this));

        if (!ros::isInitialized())
        {
            int argc = 0;
            char **argv = NULL;
            ros::init(argc, argv, _model->GetName() + "_IMU_node",ros::init_options::NoSigintHandler);
        }
    ...
    //Parameters for the filter
        ...
        if(_sdf->HasElement("link"))
        {
            ...
            std::cerr << "Loading RPY plugin to: " <<_model->GetName()<< "...\n";
        }
    ...
    //filter Initialization
        this->filtRPYPub=this->rosNode->advertise<vans_copter::RPY>(_model->GetName()+"/filtRPY", 1);
        this->model=_model;
        for(int m=0;m<2;m++)
        {
            for(int n=0;n<3;n++)
  ...
  GZ_REGISTER_MODEL_PLUGIN(RPYPlugin)
}
#endif